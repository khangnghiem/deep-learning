...
def main():
    ...
    for img_path in tqdm(test_images, desc="Evaluating SAM2 Otsu Intersection"):
        ...
        if len(boxes) > 0:
            for box in boxes:
                ...
                box_mask = extract_mask(res_box, h, w)
                
                # Local Otsu Intersection
                x1, y1, x2, y2 = map(int, box)
                x1, y1 = max(0, x1), max(0, y1)
                x2, y2 = min(w, x2), min(h, y2)
                
                box_gray = gray[y1:y2, x1:x2]
                if box_gray.size > 0:
                    otsu_thresh, box_otsu = cv2.threshold(box_gray, 0, 255, cv2.THRESH_BINARY + cv2.THRESH_OTSU)
                    cy, cx = box_gray.shape[0]//2, box_gray.shape[1]//2
                    polyp_intensity = box_gray[cy, cx]
                    
                    if polyp_intensity < otsu_thresh:
                        box_otsu = cv2.bitwise_not(box_otsu)
                    
                    full_otsu = np.zeros((h, w), dtype=np.uint8)
                    full_otsu[y1:y2, x1:x2] = box_otsu
                    
                    # Intersect SAM2 mask with Local Otsu Mask
                    box_mask = np.logical_and(box_mask, full_otsu > 0).astype(np.uint8)
