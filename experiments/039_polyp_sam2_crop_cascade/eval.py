...
def main():
    ...
    for img_path in tqdm(test_images, desc="Evaluating SAM2 Crop Cascade"):
        ...
        if len(boxes) > 0:
            for box in boxes:
                ...
                cropped_box = [box[0]-x1, box[1]-y1, box[2]-x1, box[3]-y1]

                sam_results = sam_model(cropped_img, bboxes=cropped_box, verbose=False)[0]
                if sam_results.masks is not None:
                    ...
                    if mask_data.size > 0:
                        ...
                        if mask_data.shape != (ch, cw):
                            mask_data = cv2.resize(mask_data.astype(np.float32), (cw, ch), interpolation=cv2.INTER_NEAREST)
                        
                        full_mask = np.zeros((h, w), dtype=np.uint8)
                        full_mask[y1:y2, x1:x2] = (mask_data > 0).astype(np.uint8)
                        pred_mask = np.logical_or(pred_mask, full_mask).astype(np.uint8)
