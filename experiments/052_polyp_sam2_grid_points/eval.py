...
def main():
    ...
    for img_path in tqdm(test_images, desc="SAM2 + 3x3 Grid Positive Prompts"):
        ...
        if len(boxes) > 0:
            for box in boxes:
                x1, y1, x2, y2 = box
                bw, bh = x2 - x1, y2 - y1
                # 3x3 grid at 25%, 50%, 75%
                pts = []
                for p_x in [0.25, 0.50, 0.75]:
                    for p_y in [0.25, 0.50, 0.75]:
                        pts.append([x1 + bw * p_x, y1 + bh * p_y])
                labels = [1] * 9
                res = sam_model(img, points=[pts], labels=[labels], verbose=False)[0]
