...
def main():
    ...
    for img_path in tqdm(test_images, desc="Evaluating SAM2 Box Jitter Ensemble"):
        ...
        if len(boxes) > 0:
            for box in boxes:
                ...
                boxes_jitter = [
                    box.tolist(),
                    [max(0, box[0]-0.05*bw), max(0, box[1]-0.05*bh), min(w, box[2]+0.05*bw), min(h, box[3]+0.05*bh)],
                    [box[0]+0.05*bw, box[1]+0.05*bh, box[2]-0.05*bw, box[3]-0.05*bh],
                    [min(w, box[0]+0.05*bw), box[1], min(w, box[2]+0.05*bw), box[3]],
                    [box[0], min(h, box[1]+0.05*bh), box[2], min(h, box[3]+0.05*bh)]
                ]
                
                all_masks = []
                for jbox in boxes_jitter:
                    res = sam_model(img, bboxes=jbox, verbose=False)[0]
                    m = extract_mask(res, h, w)
                    all_masks.append(m)
                
                # Majority vote
                mask_sum = np.sum(all_masks, axis=0)
                voted_mask = (mask_sum >= 3).astype(np.uint8)
                pred_mask = np.logical_or(pred_mask, voted_mask).astype(np.uint8)
