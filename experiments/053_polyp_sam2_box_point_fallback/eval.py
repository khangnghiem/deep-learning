...
def main():
    ...
    for img_path in tqdm(test_images, desc="SAM2 Box Prompt with Center Point Fallback"):
        ...
        if len(boxes) > 0:
            for box in boxes:
                ...
                # If too small (< 10% of box), fallback to Center Point
                if mask_area < 0.10 * box_area:
                    cx, cy = (x1 + x2) / 2.0, (y1 + y2) / 2.0
                    res = sam_model(img, points=[[[cx, cy]]], labels=[[1]], verbose=False)[0]
                    m = extract_mask(res, h, w)
