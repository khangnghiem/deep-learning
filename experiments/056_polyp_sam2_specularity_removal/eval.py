...
def main():
    ...
    for img_path in tqdm(test_images, desc="SAM2 with Specularity Inpainting"):
        ...
        # Dilate mask slightly to cover edges of reflections
        kernel = np.ones((3,3), np.uint8)
        mask = cv2.dilate(mask, kernel, iterations=1)
        enhanced_img = cv2.inpaint(img, mask, 3, cv2.INPAINT_NS)
